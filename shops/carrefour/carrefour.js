...
const carrefourExpress = async () => {
  try {
    ...
    fs.writeFileSync("./carrefour/carrefour.json", JSON.stringify(prodotti));
    console.log("Done!");
    await browser.close();
  } catch (error) {
    console.log(error);
...
const carrefourMarket = async () => {
  try {
    ...
    // await button.click();
    let prodotti = [];
    const listView = await page.waitForSelector(
      ".switch-view__label.label--listing"
    );
    listView.click();
    const pageEl = await page.$eval(
      ".grid-footer p.text-center",
    ...
    for (let i = 0; i <= totalPages; i++) {
      ...
      //   ({ innerText }) => innerText.split("al ")[1].trim()
      // );
      const pageProds = await scrapeCards(cards, scadenza, "carrefour-market");
      prodotti = [...prodotti, ...pageProds];
      const hasNext = await page.$(".btn-show-more");
      if (hasNext) {
        const nextButton = await page.waitForSelector(".btn-show-more");
