...
import { delay, emptyFolder } from "../../utils/index.js"
...
export const crop = async (
  ...
  try {
    ...
    Logger.level(4).debug(`Now cropping: ${fileName}`)
    ...
    Logger.level(4).debug("Page has been cropped")
  } catch (error) {
    Logger.error(error)
  }
}
      ...
      for (let i = 1; i <= images.length; i++) {
        ...
        if (ogHeight == ogWidth || ogWidth / ogHeight > 1) {
          ...
          await crop(
            ...
            width,
            height,
            path.resolve(__dirname, "..", "shops", "basko")
          )
        } else {
...
export const uploadImages = async (folder, chosenPath) => {
  ...
  const uploadPromises = files
    .filter((file) => file.includes("color"))
    .map((file) => {
      Logger.level(4).debug(`Upload... ${file}`)
